...
function IncognitoResurrected:OnInitialize()
    ...
    -- Detect API version, remember it for deferred hook setup
    self._isRetail = self:IsRetailAPI()

    -- get current character name (stored on the addon table so RetailAPI.lua/
    -- ClassicAPI.lua, which load as separate chunks, can see it too)
    self.character_name = UnitName("player")

    -- Hooking SendChatMessage/SendMessage while in combat can trip
    -- "AddOn Action Forbidden" errors, so defer until combat ends.
...
function IncognitoResurrected:SetupApiHooks()
    if self._hooksInstalled then return end
    if self._isRetail then
        self:RetailHooks()
    else
        self:ClassicHooks()
    end
...
function IncognitoResurrected:ChatPrefixColorFilter(_frame, _event, msg, author, ...)
